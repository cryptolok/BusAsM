#!/usr/bin/python2
# the file to execute in order to get the transmission
...
from custom import (FRAMES_PER_BUFFER, FORMAT, CHANNELS, RATE,
...
class Meter(object):
    ...
    def __init__(self, collect=False, seconds=None, action=None,
        ...
        self.LISTEN=False
        self.LIMIT=4000
# this is to modify after the tests you made to know for how much the power will decrease after signal is produced
        self.BIT=1
        self.bits='0b'
        self.Bits=self.bits
        self.trueBits=''
...
#            sys.stdout.flush()
# 8 times is 8 bits transmission and if no signal after then ok
            print str(rms)
            if self.LISTEN:
                if rms > self.LIMIT:
                    self.bits+='0'
                    ...
                    print 1
                if self.BIT == 8:
                    print 'DATA : '+chr(int(self.bits,2))
                    self.LISTEN=False
                self.BIT+=1
                return
            if rms < self.LIMIT:
